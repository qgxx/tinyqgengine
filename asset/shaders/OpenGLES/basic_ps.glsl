...
const vec2 _354[4] = vec2[](vec2(-0.94201624393463134765625, -0.39906215667724609375), vec2(0.94558608531951904296875, -0.768907248973846435546875), vec2(-0.094184100627899169921875, -0.929388701915740966796875), vec2(0.34495937824249267578125, 0.29387760162353515625));
...
highp vec3 apply_areaLight(Light light)
{
    ...
    highp vec3 right = normalize((_500.viewMatrix * vec4(1.0, 0.0, 0.0, 0.0)).xyz);
    highp vec3 pnormal = normalize((_500.viewMatrix * light.lightDirection).xyz);
    highp vec3 ppos = (_500.viewMatrix * light.lightPosition).xyz;
    ...
    if (light.lightShadowMapIndex != (-1))
    {
        ...
        switch (light.lightType)
        {
            ...
            case 1:
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    near_occ = texture(shadowMap, vec3(v_light_space.xy + (_354[i] / vec2(700.0)), float(light.lightShadowMapIndex))).x;
            ...
            case 2:
            {
                ...
                {
                    near_occ = texture(globalShadowMap, vec3(v_light_space.xy + (_354[i_1] / vec2(700.0)), float(light.lightShadowMapIndex))).x;
                    if ((v_light_space.z - near_occ) > bias)
                    {
                        visibility -= 0.2199999988079071044921875;
                    }
            ...
            case 3:
            {
                ...
                {
                    near_occ = texture(shadowMap, vec3(v_light_space.xy + (_354[i_2] / vec2(700.0)), float(light.lightShadowMapIndex))).x;
                    if ((v_light_space.z - near_occ) > bias)
                    {
                        visibility -= 0.2199999988079071044921875;
                    }
...
{
    highp vec3 N = normalize(normal.xyz);
    highp vec3 light_dir = normalize((_500.viewMatrix * light.lightDirection).xyz);
    highp vec3 L;
