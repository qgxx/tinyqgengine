    ...
    if (light.lightShadowMapIndex != (-1))
    {
        ...
        switch (light.lightType)
        {
            ...
            case 1:
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    near_occ = shadowMap.Sample(_shadowMap_sampler, float3(v_light_space.xy + (_354[i] / 700.0f.xx), float(light.lightShadowMapIndex))).x;
            ...
            case 2:
            {
                ...
                {
                    near_occ = globalShadowMap.Sample(_globalShadowMap_sampler, float3(v_light_space.xy + (_354[i_1] / 700.0f.xx), float(light.lightShadowMapIndex))).x;
                    if ((v_light_space.z - near_occ) > bias)
                    {
                        visibility -= 0.2199999988079071044921875f;
                    }
            ...
            case 3:
            {
                ...
                {
                    near_occ = shadowMap.Sample(_shadowMap_sampler, float3(v_light_space.xy + (_354[i_2] / 700.0f.xx), float(light.lightShadowMapIndex))).x;
                    if ((v_light_space.z - near_occ) > bias)
                    {
                        visibility -= 0.2199999988079071044921875f;
                    }
