...
vertex main0_out main0(main0_in in [[stage_in]], constant PerBatchConstants& _13 [[buffer(0)]], constant PerFrameConstants& _42 [[buffer(1)]])
{
    main0_out out = {};
    float3x3 TBN = {};
    ...
    out.normal_world = fast::normalize(_13.modelMatrix * float4(in.inputNormal, 0.0));
    out.normal = fast::normalize(_42.viewMatrix * out.normal_world);
    float3 tangent = fast::normalize(float3((_13.modelMatrix * float4(in.inputTangent, 0.0)).xyz));
    tangent = fast::normalize(tangent - (out.normal_world.xyz * dot(tangent, out.normal_world.xyz)));
